...
namespace WebAppPOSAdmin.ReportesPdf
{
    public partial class Entradas : System.Web.UI.Page
    {
        ...
        public void createPDF()
        {
            try
            {
                if (Session["EntradaID"] == null)
                {
                    return;
                }
                using dcContextoSuPlazaDataContext dcContextoSuPlazaDataContext = new dcContextoSuPlazaDataContext();
                entrada entrada = dcContextoSuPlazaDataContext.entrada.FirstOrDefault((entrada e) => e.id_entrada.Equals(Guid.Parse(Session["EntradaID"].ToString())));
                List<ArticuloExtended> entradas = new Procedures().getEntradas(Guid.Parse(Session["EntradaID"].ToString()));
                ...
                PdfPCell pdfPCell2 = new PdfPCell(new Phrase($"ENTRADA A ALMACEN\nENTRADA #: {entrada.num_entrada}, \t FECHA: {entrada.fecha_entrada}\nRESPONSABLE: {entrada.user_name}\n\nOBSERVACIONES: {entrada.observacion}", font));
                ...
                int num = 1;
                foreach (ArticuloExtended item in entradas)
                {
                ...
                decimal num2 = entradas.Sum((ArticuloExtended a) => a.total);
                ...
                foreach (var item2 in from a in entradas
                ...
                base.Response.AppendHeader("content-disposition", $"filename=Entrada_{entrada.num_entrada}.pdf");
